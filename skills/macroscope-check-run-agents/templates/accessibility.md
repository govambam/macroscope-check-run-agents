...
Clickable elements must be real buttons/links or have an appropriate `role`, be keyboard focusable, and respond to Enter/Space. Flag `onClick` handlers on non-interactive elements without keyboard support.

### Images and icons have text alternatives

Meaningful images need `alt` text; decorative ones need empty `alt`. Icon-only buttons need an accessible label (`aria-label`).
