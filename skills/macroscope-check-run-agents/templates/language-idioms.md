...
Errors must be wrapped with context using `fmt.Errorf("...: %w", err)` (Go) so the failure site is identifiable. Never discard an error with `_` unless there is an explicit comment justifying it.

### Naming

Exported identifiers use the codebase's documented casing (PascalCase for Go exports, camelCase for TypeScript). Avoid abbreviations that aren't already established in the codebase.

### Prefer shared utilities

Reach for existing helpers (slice/map utilities, formatting helpers) instead of hand-rolling loops that duplicate an established helper.
